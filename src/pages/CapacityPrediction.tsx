...
import { capacityPredictions } from '../data/mockData';
import type { ScenarioExtraction, CapacityPrediction as CapacityPredictionType } from '../types';
import {
  formatNumber,
  formatCurrency,
  getPriorityColor,
...
const CapacityPrediction: React.FC = () => {
  ...
  const selectedPrediction: CapacityPredictionType | undefined = useMemo(() => {
    return capacityPredictions.find(cp => cp.cityCode === selectedCityCode);
  }, [selectedCityCode]);
  ...
  const handleFile = useCallback((file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const scenarios: ScenarioExtraction[] = jsonData.map((row, idx) => {
          const typeRaw = String(row['场景类型'] || row['type'] || row['场景'] || 'residential').toLowerCase();
          let type: ScenarioExtraction['type'] = 'residential';
          if (typeRaw.includes('商业') || typeRaw.includes('commercial')) type = 'commercial';
          else if (typeRaw.includes('工业') || typeRaw.includes('industrial')) type = 'industrial';
          else if (typeRaw.includes('交通') || typeRaw.includes('transport')) type = 'transportation';

          return {
            type,
            name: String(row['场景名称'] || row['名称'] || row['name'] || `场景${idx + 1}`),
            estimatedUsers: Number(row['预估用户数'] || row['用户数'] || row['estimatedUsers'] || 0) || Math.floor(5000 + Math.random() * 50000),
            area: String(row['所在区域'] || row['区域'] || row['area'] || '未知区域'),
          };
        });
  ...
  const displayPrediction = selectedPrediction ?? capacityPredictions[0];
      ...
      >
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {displayPrediction.recommendedStations.map(station => (
